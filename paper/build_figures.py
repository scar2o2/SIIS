...
def generate_fig1_arch():
    ...
    draw_card(6, 4, 38, 38, c_db, c_db_b, 'Supabase Spatial Database & Storage', [
    ...
    draw_card(56, 4, 38, 38, c_admin, c_admin_b, 'Municipal Operations Command Center', [
    ...
    arrow(46, 50, 30, 42, 'Insert Defect Record\n& Storage Upload', (-3.0, 0))
    arrow(44, 25, 56, 25, 'Spatial Queries\n& Live Sync', (0, 0))
    arrow(56, 15, 44, 15, 'Status Updates\n& Task Dispatch', (0, 0))
...
def generate_fig2_detections():
    ...
    for y in range(80, 370):
        ...
        img2[y, x_c-2:x_c+2] = [20, 20, 20]
        img2[y, x_c-3:x_c-2] = [50, 50, 48]
        img2[y, x_c+2:x_c+4] = [130, 128, 120]
        if y % 32 == 0:
    ...
    img3 = np.full((h, w, 3), 115, dtype=np.uint8)
    img3[140:144, :] = 80
    img3[280:284, :] = 80
    img3[310:, :] = (asphalt_rgb[310:, :] * 0.85).astype(np.uint8)
    img3[305:310, :] = 160

    img3[70:270, 70:220] = [38, 106, 60]
    for rx in [105, 145, 185]:
        img3[70:270, rx-2:rx+2] = [25, 75, 40]
    img3[65:75, 65:225] = [28, 85, 45]

    trash_pile = (((xx - 145)**2)/(60**2) + ((yy - 60)**2)/(35**2)) < 1.0
    img3[trash_pile] = [215, 205, 185]
    bag1 = (((xx - 120)**2)/(18**2) + ((yy - 55)**2)/(15**2)) < 1.0
    img3[bag1] = [30, 130, 200]
    bag2 = (((xx - 165)**2)/(20**2) + ((yy - 62)**2)/(16**2)) < 1.0
    img3[bag2] = [210, 50, 45]

    litter = (((xx - 310)**2)/(70**2) + ((yy - 365)**2)/(40**2)) < 1.0
    img3[litter] = [220, 215, 195]
    litter_can = (((xx - 330)**2)/(12**2) + ((yy - 360)**2)/(8**2)) < 1.0
...
def generate_fig5_admin_map():
    ...
    ax.text(97.5, 95.5, chr(9679) + ' System Live  |  Admin: M. Cherukuri', fontsize=7.5,
    ...
    ax.text(37.5, 53.5, 'Coords: 13.0612 deg N, 80.2584 deg E (Anna Salai)', fontsize=6.6, color='#94A3B8', zorder=11)
